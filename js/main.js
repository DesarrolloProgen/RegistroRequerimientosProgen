window.addEventListener("load", function () {
  ...
  const url =
    "https://prod-23.brazilsouth.logic.azure.com:443/workflows/173e479768634895b9603f0acaa8927f/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=9z6PMT_1dXq1JSpLLW3S1bRKjCrPq-B4Sy84LsvaLzc&pais=Colombia";
  var data = {};
  /*-------------------------------------------------------------------------------------------- */
  ...
  form.addEventListener("submit", function (event) {
    ...
    if (
      c_requerimiento.value == "Reclamo / Producto No Conforme" ||
      c_requerimiento.value == "Queja"
    ) {
      data = {
        fecha: fechayHora,
        nombre: nombre.value,
        ...
        archivoFactura: "",
        punto_venta: "",
        linea: param_division_comercial
          ? param_division_comercial
          : division_comercial.value,
        linea_comercial: "",
        mayorista: "",
      };
    } else {
    ...
    fetch(url, settings)
      .then(async (response) => {
        ...
        document.getElementById("mensaje").innerHTML =
          "Se realizó correctamente su registro. La PQRSF asignada a su solicitud es: " +
          resp.registro;
      ...
      .catch((err) => {
        ...
        document.getElementById("mensaje").innerHTML =
          "Algo salio mal.... Recargue la pagina e intete nuevamente<br>" + err;
      });
  }
  ...
  function obtenerDatos() {
    var url =
      "https://prod-28.brazilsouth.logic.azure.com:443/workflows/be2d5679275e4f098a00f40ec0f04a29/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=P66ODxcUOPOzaI2KUyOHPF0AYDrTRl1nHU-XhAAI94o&pais=colombia";
    fetch(url)
      .then((response) => {
        return response.json();
      })
      .then(function (data) {
        ...
        for (var i = 0; i < data.value.length; i++) {
          if (data.value[i].Division_Comercial != "") {
            listDivCom +=
              "<option value='" +
              data.value[i].Division_Comercial +
              "'>" +
              data.value[i].Division_Comercial +
              "</option>";
          }
        ...
        if (param_division_comercial) {
          if (param_division_comercial == "Instrumentación") {
            ...
            document.querySelector("#t_cliente").innerHTML = lisTipoClienteIns;
          }
        } else {
